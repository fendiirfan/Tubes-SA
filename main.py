...
getCoordinateLink = "https://api.openrouteservice.org/geocode/search?api_key={}&text=".format(
    API_KEY)
getMatrixLink = ""
    ...
    for i in range(len(list_destinasi) - 1):
        if len(rute_akhir) == 1:
            next_destinasi_kanan = list_destinasi[
                rute_akhir[0]].jarakMinimumSelainList(rute_akhir)
            rute_akhir.append(next_destinasi_kanan[0])
        else:
            next_destinasi_kanan = list_destinasi[rute_akhir[
                len(rute_akhir) - 1]].jarakMinimumSelainList(rute_akhir)
            next_destinasi_kiri = list_destinasi[
                rute_akhir[0]].jarakMinimumSelainList(rute_akhir)
